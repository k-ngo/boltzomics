...
class ScreeningJobManager:
    ...
    def register_executor(
        ...
        try:
            ...
            self._executor_accepts_worker_id = len(positional) >= 2
        except Exception:
            self._executor_accepts_worker_id = False
        self._ensure_workers()

    def set_worker_count(self, count: int) -> None:
        """Configure how many queue workers can execute jobs concurrently."""
    ...
    def get_project_summary(self, project_name: str) -> Dict[str, Any]:
        ...
        summary["active_job"] = running_jobs[0] if running_jobs else None
        summary["active_jobs"] = running_jobs
    ...
    def _ensure_workers(self) -> None:
        ...
        for worker_id in range(target_workers):
            existing = self.worker_threads.get(worker_id)
    ...
    def _worker_loop(self, worker_id: int) -> None:
        while not self.shutdown_event.is_set():
            if worker_id >= max(1, int(self.worker_count)):
            ...
            if job is None:
                ...
                continue
            try:
                if self._executor_accepts_worker_id:
                    result, metadata = self.executor(job, worker_id)
                else:
